...
def status():
    ...
    mode = "🎭 DEMO" if config.IS_DEMO else "🚀 PRODUÇÃO"
    config_text = f"""
Modo: {mode}
...
Database: {config.DATABASE_URL}
    """
    console.print(Panel(config_text, title="[bold blue]Configuração[/bold blue]", box=box.ROUNDED))
    ...
    table.add_row("Total de Produtos", str(stats['total_products']))
    table.add_row("Produtos Hoje", str(stats['products_today']))
    table.add_row("Total de Vídeos", str(stats['total_videos']))
    table.add_row("Vídeos Publicados", str(stats['videos_published']))
    ...
    apis = [
        ("OpenAI", "✅" if config.OPENAI_API_KEY != "demo-key" else "🎭 DEMO"),
        ("Suno", "✅" if config.SUNO_API_KEY != "demo-key" else "🎭 DEMO"),
        ("YouTube", "✅" if config.YOUTUBE_CLIENT_ID else "❌"),
        ("Shopee", "✅" if config.SHOPEE_AFFILIATE_ID else "❌"),
    ]
...
def demo():
    ...
    console.print(Panel.fit(
        "[bold green]🚀 AgentMoney System - Modo Demonstração[/bold green]\n"
        "Todas as APIs estão em modo simulação.\n"
        "Execute 'cp .env.example .env' e configure para produção.",
        box=box.DOUBLE
    ))
    ...
    console.print("Use [bold]python main.py status[/bold] para ver resultados.")
